...
// Přejmenováno z DottedLineRasterizerTrivial
public class DashRasterizer implements Rasterizer {

    private Raster raster;

    public DashRasterizer(Raster raster) {
        this.raster = raster;
    }
    ...
    public void rasterize(Line line) {
        ...
        int err = dx - dy;
        int dashCounter = 0;
        int dashLength = 10; // Délka každého úseku

        while (true) {
            // Vykreslení úseku pouze pokud je čítač v rámci délky úseku
            if (dashCounter < dashLength) {
                drawThickPixel(x1, y1, color, lineWidth);
            }
            
            dashCounter = (dashCounter + 1) % (dashLength * 2);
